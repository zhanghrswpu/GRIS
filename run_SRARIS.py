...
from models.SRARIS import SRARIS
np.random.seed(2024)
...
def parse_args():
    ...
    parser.add_argument('--num_neg', type=int, default=10, help='number of negetiva samples for each [u,i] pair')

    ### model parameters ###
    parser.add_argument('--gcn_layer', type=int, default=4, help='?')
    ...
    parser.add_argument('--feature', type=float, default=2.0, help='weight of Feature_loss')
...
def save_file(save_path):
    copyfile('./models/SRARIS.py', save_path + 'SRARIS.py')
    copyfile('./run_SRARIS.py', save_path + 'run_SRARIS.py')
...
if __name__ == '__main__':
    ...
    record_path = './saved/' + args.dataset + '/SRARIS/' + args.runid + '/'
    ...
    rec_model = SRARIS(args, rec_data)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch % 1) == 0:
            ...
            if epoch > 10 and early_stop > args.early_stops:
